...
Test embed URLs for the live hosts this backend supports (used to verify
`POST /api/extract` → `/proxy` chains). Paste any into the player at `/?url=<embed>`.

| SR  | Host               | Name       | Embed URL                                | Status |
|-----|--------------------|------------|------------------------------------------|--------|
| SR2 | vidhideplus.com    | Vidhide    | https://vidhideplus.com/v/jehmieb1epa5   | ✅ works (local only; 403 on CF) |
